...
contract PluginRepoFactory {
    ...
    /// @notice Initializes the addresses of the plugin registry and `PluginRepo` base contract to proxy to.
    ...
    /// @notice Creates a plugin repository proxy pointing to the `pluginRepoBase` implementation and registers it in the plugin registry.
    ...
    /// @notice Internal method creating a `PluginRepo` via the [ERC-1967](https://eips.ethereum.org/EIPS/eip-1967) proxy pattern from the provided base contract and registering it in the plugin registry.
